"""Ed25519/Ed448 reference implementation from RFC 8032 Appendix A.

Source: https://www.rfc-editor.org/rfc/rfc8032.txt (public domain)
Vendored verbatim from RFC text, 2026-06-21.

WARNING (per RFC 8032 itself):
  This is the educational reference impl. Slow, no side-channel
  hardening. For skill-sign use case (sign ~200 bytes once per
  skill publish, offline) this is acceptable. For high-frequency
  or hostile environments swap for cryptography.hazmat or PyNaCl.

skill-sign only uses Ed25519 (via Ed25519.keygen / .sign / .verify).
Ed448 code is retained verbatim from RFC for code-integrity reasons.
"""
